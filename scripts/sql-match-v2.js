...
export const TRADE_MATCH_GRID = `
  ...
  FROM (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY t.sell_history_id ORDER BY ABS(CAST(t.sell_entry_price AS DOUBLE) - CAST(t.buy_entry_price AS DOUBLE))) AS sell_seq
    FROM (
      SELECT
        ...
        -- 计算持有天数和价格差
        CAST((t1.entry_date_time - t2.entry_date_time) / INTERVAL '1 day' AS INTEGER) AS days_diff,
